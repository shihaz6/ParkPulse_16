  ...
  const goNext = async () => {
    ...
    if (step === 5) {
      ...
      try {
        ...
        setMemberQrCode(created.qrCode || '');
        setStep(6);
      } catch (err: any) {
        setProcessing(false);
              ...
              {memberQrCode && (
                <div className="w-full bg-background border border-border rounded-2xl overflow-hidden text-left">
                  <div className="px-4 py-3 border-b border-border bg-muted/20 flex items-center justify-between">
                    <p className="text-xs text-muted-foreground uppercase tracking-widest">Membership QR Code</p>
                    <button
                      onClick={() => {
                        const link = document.createElement('a');
                        link.href = memberQrCode;
                        link.download = `parkpulse-${finalCreds.username}-qrcode.png`;
                        link.click();
                      }}
