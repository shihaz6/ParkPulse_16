...
import {
  ...
  CheckCircle2, Clock, AlertTriangle, CreditCard, Lock, AtSign, Eye, EyeOff, QrCode,
} from 'lucide-react';
...
export function MembersView({
  ...
  const [qrView, setQrView] = useState<Member | null>(null);
                    ...
                    <button
                      onClick={() => {
                        if (member.qrCode) {
                          ...
                          memberService.getMemberQrCode(member.id)
                            .then(res => setQrView({ ...member, qrCode: res.qrCode }))
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm" onClick={() => setQrView(null)}>
          <div className="bg-card border border-border rounded-2xl w-full max-w-sm shadow-2xl" onClick={e => e.stopPropagation()}>
            <div className="px-5 py-4 border-b border-border flex items-center justify-between">
              <p className="font-semibold text-sm">{qrView.name}'s QR Code</p>
              <button onClick={() => setQrView(null)} className="text-muted-foreground hover:text-foreground transition-colors">
                <X className="w-4 h-4" />
              </button>
            </div>
            <div className="p-6 flex flex-col items-center gap-4">
              {qrView.qrCode ? (
                <>
                  <img src={qrView.qrCode} alt="QR Code" className="w-56 h-56 rounded-xl border border-border" />
                  <button
                    onClick={() => {
                      const link = document.createElement('a');
                      link.href = qrView.qrCode!;
                      link.download = `parkpulse-${qrView.username || qrView.id}-qrcode.png`;
                      link.click();
                    }}
