...
function LoginPage({
  ...
  return (
    ...
    {requestOpen && <RequestAccessModal onClose={() => setRequestOpen(false)} plans={plans} />}
    </>
  );
}
...
function RequestAccessModal({ onClose, plans }: { onClose: () => void; plans: MembershipPlan[] }) {
  ...
  const goNext = () => {
    ...
    if (step === 5) {
      ...
      setTimeout(() => {
        setProcessing(false);
        const creds = { username: genUsername(form.name), password: genPassword() };
        setFinalCreds(creds);
        setStep(6);
      }, 1600);
      return;
              ...
              <div className="bg-background border border-border rounded-xl p-4 flex items-center justify-between">
                ...
                <div className="text-right">
                  ...
                  <p className="font-bold text-blue-500">
                    රු{parseFloat((plans.find(p => p.name === selectedPlan)?.[billing === 'monthly' ? 'monthlyPrice' : 'annualPrice'] ?? '0')).toFixed(2)}
                  </p>
                </div>
              </div>
...
function GeneralSettings({ darkMode, onToggleDarkMode, currency, onCurrencyChange }: {
    ...
    <div className="space-y-6">
      ...
      <div className="bg-card rounded-2xl p-6 space-y-5 border border-border">
        <div>
          ...
          <input defaultValue="ParkPulse Main Lot" className="w-full bg-background text-foreground rounded-lg px-4 py-3 outline-none border border-border focus:border-blue-600 transition-colors" />
        </div>
        <div>
          <label className="block text-sm text-muted-foreground mb-2">Timezone</label>
          <select className="w-full bg-background text-foreground rounded-lg px-4 py-3 outline-none border border-border focus:border-blue-600 transition-colors">
      ...
      <button className="px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-lg transition-colors">Save Changes</button>
    </div>
  );
}
...
function blankZone(): Zone {
  return { id: '', name: '', prefix: '', totalSlots: 10, reservedSlots: 0, ratePerHour: '10.00', rateType: 'Flat hourly rate', vehicleTypes: ['Car'], overflowAlert: true, autoRelease: false, releaseTimeout: '120', color: 'blue', status: 'active' };
}
...
function ParkingSlotSettings({ currencySymbol, zones, setZones }: {
  ...
  const save = (z: Zone) => {
    if (mode === 'create') {
      setZones(prev => [...prev, { ...z, id: `z${Date.now()}` }]);
    } else {
      setZones(prev => prev.map(existing => existing.id === z.id ? z : existing));
    }
    setMode('list');
    setEditing(null);
  };
...
const CUSTOM_PERMISSION_GROUPS = [
  {
    ...
    items: [
      ...
      { id: 'reports-admin',     label: 'Reports - Admin',    desc: 'View and add reports' },
      { id: 'reports-staff',     label: 'Reports - Staff',    desc: 'View reports only' },
...
function AccessControl({ staff, setStaff }: { staff: StaffMember[]; setStaff: React.Dispatch<React.SetStateAction<StaffMember[]>> }) {
  ...
  const openCreate = () => { setEditing(blankStaff()); setMode('create'); };
  const openEdit = (s: StaffMember) => { setEditing({ ...s }); setMode('edit'); };
  const cancel = () => { setMode('list'); setEditing(null); };

  const save = (s: StaffMember) => {
    if (mode === 'create') {
  ...
  const toggleActive = (id: string) => setStaff(prev => prev.map(s => s.id === id ? { ...s, active: !s.active } : s));
...
const DEFAULT_PLANS: MembershipPlan[] = [
  {
    id: 'p1', name: 'Free', description: 'Perfect for trying out ParkPulse',
    monthlyPrice: '0.00', annualPrice: '0.00', color: 'blue',
    features: ['1 parking slot', 'Basic dashboard', 'Email support'],
    maxVehicles: 1, status: 'active', popular: false,
  },
  {
    id: 'p2', name: 'Basic', description: 'Great for individuals and small users',
    monthlyPrice: '9.99', annualPrice: '99.99', color: 'emerald',
    features: ['3 parking slots', 'Live feed', 'Analytics', 'Email & SMS alerts'],
    maxVehicles: 3, status: 'active', popular: false,
  },
  {
    id: 'p3', name: 'Professional', description: 'Ideal for growing teams and facilities',
    monthlyPrice: '24.99', annualPrice: '249.99', color: 'violet',
    features: ['10 parking slots', 'Priority support', 'Advanced analytics', 'Custom zones', 'Revenue reports'],
    maxVehicles: 10, status: 'active', popular: true,
  },
  {
    id: 'p4', name: 'Enterprise', description: 'Full power for large parking operations',
    monthlyPrice: '59.99', annualPrice: '599.99', color: 'amber',
    features: ['Unlimited slots', 'Dedicated support', 'Custom integrations', 'SLA guarantee', 'API access'],
    maxVehicles: 'unlimited', status: 'active', popular: false,
  },
];
...
function MembershipPlans({ currencySymbol, plans, setPlans }: { currencySymbol: string; plans: MembershipPlan[]; setPlans: React.Dispatch<React.SetStateAction<MembershipPlan[]>> }) {
  ...
  const save = (p: MembershipPlan) => {
    if (mode === 'create') {
      setPlans(prev => [...prev, { ...p, id: `p${Date.now()}` }]);
    } else {
      setPlans(prev => prev.map(existing => existing.id === p.id ? p : existing));
    }
...
const DEFAULT_MEMBERS: Member[] = [
  { id: 'm1', name: 'Sarah Chen',      username: 'sarah.chen',   phone: '+94 771 001 001', email: 'sarah@email.com',   plan: 'Professional', vehicles: 2, joined: 'Jan 12, 2026', status: 'active'    },
  { id: 'm2', name: 'David Park',      username: 'david.park',   phone: '+94 771 001 002', email: 'david@email.com',   plan: 'Basic',        vehicles: 1, joined: 'Feb 3, 2026',  status: 'active'    },
  { id: 'm3', name: 'Maria Rodriguez', username: 'maria.r',      phone: '+94 771 001 003', email: 'maria@email.com',   plan: 'Enterprise',   vehicles: 4, joined: 'Mar 7, 2026',  status: 'active'    },
  { id: 'm4', name: 'James Wilson',    username: 'james.w',      phone: '+94 771 001 004', email: 'james@email.com',   plan: 'Free',         vehicles: 1, joined: 'Mar 19, 2026', status: 'pending'   },
  { id: 'm5', name: 'Priya Sharma',    username: 'priya.sharma', phone: '+94 771 001 005', email: 'priya@email.com',   plan: 'Basic',        vehicles: 2, joined: 'Apr 2, 2026',  status: 'active'    },
  { id: 'm6', name: 'Tom Nakamura',    username: 'tom.n',        phone: '+94 771 001 006', email: 'tom@email.com',     plan: 'Professional', vehicles: 3, joined: 'Apr 14, 2026', status: 'suspended' },
];
...
function ProfileSettings({ profileName, profileAvatar, onSaveProfile }: {
  ...
  return (
    <div className="space-y-6">
      ...
      <button onClick={handleSave} className={`px-6 py-3 rounded-lg transition-all font-medium text-white ${saved ? 'bg-emerald-500' : 'bg-blue-600 hover:bg-blue-700'}`}>
    ...
    <div className="space-y-6">
      ...
      {step === 'verify' ? (
        <div className="bg-card rounded-2xl p-6 space-y-5 border border-border">
          ...
          <button onClick={verify} className="w-full py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-medium transition-colors">
            Verify & Continue
          </button>
        </div>
      ) : (
        <div className="bg-card rounded-2xl p-6 space-y-5 border border-border">
          ...
          <button onClick={save} disabled={success}
            className={`w-full py-3 rounded-lg font-medium transition-all text-white ${success ? 'bg-emerald-500' : 'bg-blue-600 hover:bg-blue-700'}`}>
...
export default function App() {
  ...
  const dailyRevenue = occupiedSlots * hourlyRate;
  ...
  const toggleSlot = (slotId: string) => {
    setSlots(prev => prev.map(slot => {
      if (slot.id === slotId) {
        ...
        if (newOccupied) {
          const vehicle = `VEH-${Math.random().toString(36).substr(2, 6).toUpperCase()}`;
          const entryTime = new Date();
          setSessions(prev => [...prev, { slotId, vehicle, entryTime }]);
          return { ...slot, occupied: true, vehicle, entryTime };
        } else {
          setSessions(prev => prev.filter(s => s.slotId !== slotId));
          return { ...slot, occupied: false, vehicle: undefined, entryTime: undefined };
        }
      }
      return slot;
    }));
  ...
  const settingsSubItems = [
    ...
    { id: 'parking-slots',      label: 'Slot Settings',     icon: ParkingSquare },
    { id: 'access-control',     label: 'Access Control',    icon: KeyRound },
    { id: 'membership-plans',   label: 'Membership Plans',  icon: CreditCard },
    { id: 'member-accounts',    label: 'Member Accounts',   icon: Users },
    { id: 'reservation-history', label: 'Reservation History', icon: BookMarked },
    { id: 'profile',            label: 'Profile',             icon: User },
    { id: 'password',           label: 'Security',            icon: Lock },
  ];

  if (!loggedIn) {
    return <LoginPage onLogin={() => setLoggedIn(true)} darkMode={darkMode} onToggleTheme={() => setDarkMode(v => !v)} plans={plans} />;
  }
...
{/* Members */}
          ...
          <div>
            ...
            {sidebarExpanded && settingsOpen && (
              <div className="mt-0.5 ml-3 space-y-0.5 border-l border-sidebar-border pl-2.5">
                {settingsSubItems.map(({ id, label, icon: Icon }) => (
                  <button
                    key={id}
        ...
        >
          ...
          {activeView === 'settings' && (
            <div className="max-w-3xl mx-auto w-full pb-4">
              {activeSettings === 'general'           && <GeneralSettings darkMode={darkMode} onToggleDarkMode={() => setDarkMode(v => !v)} currency={currency} onCurrencyChange={setCurrency} />}
              {activeSettings === 'security'          && <SecuritySettings />}
              {activeSettings === 'parking-slots'     && <ParkingSlotSettings currencySymbol={currencySymbol} zones={zones} setZones={setZones} />}
              {activeSettings === 'access-control'    && <AccessControl staff={staff} setStaff={setStaff} />}
              {activeSettings === 'membership-plans'  && <MembershipPlans currencySymbol={currencySymbol} plans={plans} setPlans={setPlans} />}
              {activeSettings === 'member-accounts'   && <MemberAccounts members={members} setMembers={setMembers} plans={plans} />}
              {activeSettings === 'reservation-history' && <ReservationHistory />}
              ...
              {activeSettings === 'password'          && <PasswordSecurity />}
            </div>
          )}
          ...
          {activeView === 'reports' && (
            <div className="max-w-[1400px] mx-auto w-full">
              <ReportsView subView={activeReportsSubView} />
            </div>
          )}
          ...
          {activeView === 'slots' && (
            <div className="max-w-[1400px] mx-auto w-full">
              <ParkingSlotsView
                ...
                members={members.map(m => ({ id: m.id, name: m.name, email: m.email, plan: m.plan }))}
          ...
          {activeView === 'dashboard' && (
            <div className="max-w-[1400px] mx-auto w-full flex flex-col gap-4 h-full">
              ...
              <div className="grid grid-cols-3 gap-4 flex-shrink-0">
                {[
                  { label: 'Total Zones',   value: String(totalZones),              sub: `${totalSlots} slots total`,  Icon: Circle as React.ElementType | null, iconText: null as string | null, iconBg: 'bg-blue-500/10',   iconColor: 'text-blue-500' },
                  { label: 'Occupied',      value: `${totalSlots > 0 ? ((occupiedSlots / totalSlots) * 100).toFixed(1) : '0.0'}%`, sub: `${occupiedSlots} of ${totalSlots} slots`, Icon: User as React.ElementType | null, iconText: null as string | null, iconBg: 'bg-orange-500/10', iconColor: 'text-orange-500' },
                  { label: 'Daily Revenue', value: `${currencySymbol}${dailyRevenue.toFixed(2)}`,   sub: 'earned today',               Icon: null,      iconText: currencySymbol,       iconBg: 'bg-green-500/10',  iconColor: 'text-green-500' },
                ].map(({ label, value, sub, Icon, iconText, iconBg, iconColor }) => (
                  <div key={label} className="bg-card rounded-2xl p-4 border border-border flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      <p className="text-xs font-medium text-muted-foreground uppercase tracking-wider mb-2">{label}</p>
                      <p className="text-2xl font-bold tracking-tight leading-none mb-1 truncate">{value}</p>
                      <p className="text-xs text-muted-foreground">{sub}</p>
                    </div>
                    <div className={`w-10 h-10 ${iconBg} rounded-xl flex items-center justify-center flex-shrink-0`}>
                      {iconText
                        ? <span className={`text-sm font-bold ${iconColor}`}>{iconText}</span>
              ...
              <div className="grid grid-cols-[1fr_320px] gap-4 flex-1 min-h-0">
                {/* Parking map */}
                <div className="bg-card rounded-2xl border border-border overflow-hidden flex flex-col min-h-0">
                  <div className="px-6 py-4 border-b border-border flex items-center justify-between flex-shrink-0">
                    <h2 className="font-semibold">Live Parking Map</h2>
                    <div className="flex items-center gap-4 text-xs text-muted-foreground">
                      ...
                      <span className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-sm bg-red-500 inline-block" />Occupied</span>
                    </div>
                  </div>
                    ...
                    <div className="grid grid-cols-8 gap-2">
                      {slots.map(slot => (
                        <button
                          ...
                          className={`aspect-[3/2] rounded-lg text-xs font-semibold transition-all text-white shadow-sm ${
                            slot.occupied
                ...
                <div className="bg-card rounded-2xl border border-border overflow-hidden flex flex-col min-h-0">
                  ...
                  <div className="overflow-y-auto flex-1 min-h-0">
                    {sessions.length === 0 ? (
                      <div className="text-muted-foreground text-center text-sm py-16 px-5">
                        No active sessions.<br />Click a slot to register an entry.
                      </div>
                    ) : (
                      <div className="p-3 space-y-2">
                        {sessions.slice().reverse().map((session, idx) => (
                          <div key={idx} className="bg-muted rounded-xl px-4 py-3 border border-border">
                            <div className="flex items-center justify-between mb-1">
                              <span className="text-sm font-semibold">{session.slotId}</span>
                              <span className="text-[11px] text-muted-foreground tabular-nums">{session.entryTime.toLocaleTimeString()}</span>
                            </div>
                            <div className="text-xs text-muted-foreground font-mono">{session.vehicle}</div>
                          </div>
                        ))}
                      </div>
                    )}
